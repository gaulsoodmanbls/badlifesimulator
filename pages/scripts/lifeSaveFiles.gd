...
func deleteLife(saveFile): #deletes ONE save file (specify file NAME (extensionless))
	print("button pressed - delete mode - " + saveFile)
	var dir = global.directoryGetter() #get the directory and make sure it exists


func buttonPressed(saveFile):
...
func _ready() -> void:
	...
	for i in dir.size(): #runs through every file in the directory
		...
		button.pressed.connect(buttonPressed.bind(fileBasename))
